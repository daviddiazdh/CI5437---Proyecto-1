...
long long find_index(vector<int> state){
    ...
    for(int i = 0; i < state.size(); i++){
        for(int j = 0; j < pattern_2.size(); j++){
            if(state[i] == pattern_2[j]){
    ...
    return lehmer_index + combinadic_index * factorial[pattern_size_2];
...
void breadth_first_search (vector<int> initial, ofstream &output_bin) {
    ...
    long long total_size = C[N][pattern_size_2] * factorial[pattern_size_2];
...
int main(){
    ...
    for(int i = 0; i < N; i++){
        for(int j = 0; j < pattern_size_2; j++){
            if(pattern_2[j] == i + 1){
                initial_state[i] = i + 1;
                break;
            } else {
